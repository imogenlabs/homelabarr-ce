...
app.get('/ports/check', requireAuth, async (req, res) => {
  try {
    ...
    try {
      // Get container port mappings using Docker CLI (works on both Linux and Windows)
      ...
      for (const line of lines) {
        try {
          ...
          // Extract ports from the Ports field (e.g., "0.0.0.0:8080->80/tcp")
          if (container.Ports) {
      ...
      // Docker not available, return template mode
      logger.warn('Docker CLI not available, using fallback:', dockerError.message);
      return res.json({
        success: true,
        usedPorts: [30002, 8083], // Current ports
        docker: {
          status: 'template-mode',
          message: 'Docker unavailable - using template mode'
        },
...
// Helper functions for parsing CLI-format stats
function parseMemoryUsage(memoryString) {
  // Parse format like "150MiB / 7.7GiB" or "150MB / 7.7GB"
...
function parseNetworkUsage(networkString) {
  // Parse format like "1.2kB / 850B" (incoming / outgoing)
...
function parseBytes(bytesString) {
  // Parse format like "150MiB", "1.2GB", "850B"
...
app.get('/containers', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      // Parse JSON output from Docker CLI
      if (result && result.trim()) {
        const lines = result.trim().split('\n');
        containers = lines.map(line => {
          try {
            const container = JSON.parse(line);
            // Transform CLI format to match expected format
            return {
              Id: container.ID,
    ...
    const containersWithStats = await Promise.all(
      containers.map(async (container) => {
        try {
          // Use CLI-based approach for both stats and inspect
